...
class Controller extends Seopanel
{
    ...
    public $sessionCats = array('common','login','button','label');
    ...
    public function assignLangCode($langCode)
    {
    ...
    public function __getAllSettings($showCheck=false, $showVal=1, $category='system')
    {
    ...
    public function defineAllSystemSettings()
    {
    ...
    public function render($viewFile='home', $layout='default')
    {
    ...
    public function pluginRender($viewFile='home', $layout='default')
    {
        if (empty($layout) || ($layout == 'default')) {
            if (!empty($this->layout)) {
                $layout = $this->layout;
    ...
    public function getLanguageTexts($category, $langCode='en')
    {
        $langTexts = array();
    ...
    public function assignTextsToSession($langCode='en', $force=false)
    {
    ...
    public function checkUserIsObjectOwner($objId, $objName='website')
    {
        if (!isAdmin()) {
            $userId = isLoggedIn();
            switch ($objName) {
    ...
    public function checkDBConn($force=false)
    {
