...
class Validation
{
    ...
    public function getUniqueChars($entry)
    {
        $arrVar = preg_split("//", $entry);
        return array_unique($arrVar);
    }

    public function checkBlank($entry)
    {
        if (strlen($entry) == 0) {
    ...
    public function checkAlpha($entry)
    {
    ...
    public function checkUname($user_name)
    {
    ...
    public function checkName($entry)
    {
        $entry = stripslashes(trim($entry));
    ...
    public function checkLastName($entry)
    {
        $entry = stripslashes(trim($entry));
        if (!preg_match($this->Filters['name'], $entry)) {
            $msg = $_SESSION['text']['common']['Invalid characters'];
    ...
    public function checkEmail($entry)
    {
        $entry = stripslashes(trim($entry));
    ...
    public function checkGenName($entry)
    {
        $entry = stripslashes(trim($entry));
    ...
    public function checkPhone($entry)
    {
        if (count($this->getUniqueChars($entry)) <= 2) {
    ...
    public function checkZip($entry)
    {
        $entry = stripslashes(trim($entry));
    ...
    public function checkNumber($entry)
    {
        $entry = stripslashes(trim($entry));
    ...
    public function checkUnameLength($name)
    {
        if (strlen($name) < 6) {
    ...
    # func to check date
    public function checkDate($date, $delimiter = '-')
    {
        $msg = '';
