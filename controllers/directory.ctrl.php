...
{
    public $noTitles = 5;              # no of titles and description for submission
    public $capchaFile = "captcha";  # captcha file name
    public $checkPR = 0;
    ...
    public function __getDirectoryInfo($dirId)
    {
    ...
    public function showWebsiteSubmissionPage($submitInfo, $error=false)
    {
    ...
    public function saveSubmissiondata($submitInfo)
    {
    ...
    public function isCategoryExists($page, $categoryCol)
    {
        $matches = array();
    ...
    public function __getCreatedCaptchaUrl($captchaUrl, $submitUrl, $phpsessid)
    {
        $spider = new Spider();
    ...
    public function startSubmission($websiteId, $dirId='')
    {
        ...
        if (!empty($_SESSION['no_reciprocal'])) {
            $sql .= " and extra_val not like '%LINK_TYPE=reciprocal%'";
        }
        if (!empty($dirId)) {
            $sql .= " and id=$dirId";
        }
    ...
    public function __getSubmitTitleDes($websiteInfo)
    {
        $titleList = array();
    ...
    public function submitSite($submitInfo)
    {
    ...
    # to skip submission
    public function skipSubmission($info)
    {
    ...
    public function unSkipSubmission($skipId)
    {
    ...
    public function __getAllSkippedDir($websiteId)
    {
    ...
    public function showSkippedDirectories($searchInfo='')
    {
        $userId = isLoggedIn();
        $websiteController = new WebsiteController();
    ...
    public function showSubmissionReports($searchInfo='')
    {
        $userId = isLoggedIn();
        $websiteController = new WebsiteController();
        $websiteList = $websiteController->__getAllWebsites($userId, true);
    ...
    public function changeConfirmStatus($dirInfo)
    {
    ...
    public function showConfirmStatus($id)
    {
        $id = intval($id);
    ...
    public function checkSubmissionStatus($dirInfo)
    {
        $dirInfo['id'] = intval($dirInfo['id']);
    ...
    public function updateSubmissionStatus($dirId, $status)
    {
    ...
    public function showSubmissionStatus($id)
    {
        $id = intval($id);
    ...
    public function checkSubmissionReports($searchInfo)
    {
        $userId = isLoggedIn();
        $websiteController = new WebsiteController();
        $this->set('websiteList', $websiteController->__getAllWebsites($userId, true));
    ...
    public function generateSubmissionReports($searchInfo)
    {
    ...
    public function deleteSubmissionReports($dirSubId)
    {
    ...
    public function showFeaturedSubmission($info="")
    {
    ...
    public function getAllFeaturedDirectories()
    {
    ...
    public function getAllDirectories($searchInfo=array())
    {
    ...
    public function getDirectoryInfo($dirId)
    {
        $dirId = intval($dirId);
        $sql = "SELECT * FROM directories where id=$dirId";
        $dirInfo = $this->db->select($sql, true);
    ...
    public function showDirectoryManager($info='')
    {
    ...
    public function changeStatusDirectory($dirId, $status, $printLink=false)
    {
        $status = intval($status);
        $dirId = intval($dirId);
    ...
    public function showCheckDirectory()
    {
        $this->render('directory/showcheckdir');
    }
    
    # function to start directory check
    public function startDirectoryCheckStatus($info='')
    {
        $searchInfo = array();
    ...
    public function checkDirectoryStatus($dirId, $nodebug=0)
    {
        $dirId = intval($dirId);
    ...
    public function getStatusLink($dirId, $status)
    {
        if ($status) {
            $statLabel = "Active";
            $statVal = 0;
    ...
    public function __getTotalSubmitInfo($websiteId, $activeCheck=false)
    {
    ...
    public function logSubmissionResult($content, $dirId, $websiteId)
    {
    ...
    public function getDirectoryScriptMetaInfo($id)
    {
        $id = empty($id) ? 1 : $id;
