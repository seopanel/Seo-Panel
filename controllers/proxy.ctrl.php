...
class ProxyController extends Controller
{
    ...
    public function listProxy($info='')
    {
        $userId = isLoggedIn();
    ...
    public function showImportProxy($info = '')
    {
        $this->render('proxy/importproxy');
    }
    
    #funvtion to import proxy
    public function importProxy($data = "")
    {
    ...
    public function showcheckAllStatus($info='')
    {
        $this->render('proxy/showcheckallstatus');
    }
    
    # function to check all proxy status
    public function checkAllProxyStatus($info = '')
    {
        $isStatusCheck = false;
        $this->updateProxyCheckedStatus(0, "");
        if ($info['status'] != "") {
    ...
    # func to check status of all proxy list
    public function runCheckStatus($info = '')
    {
    ...
    public function updateProxyCheckedStatus($checkedVal = 0, $where = "")
    {
    ...
    public function __getProxyCount($where = '')
    {
    ...
    public function __getAllProxys($isStatusCheck=true, $status=1)
    {
        $sql = "select * from proxylist where 1=1";
        if ($isStatusCheck) {
    ...
    public function __changeStatus($proxyId, $status)
    {
    ...
    # func to change status
    public function __deleteProxy($proxyId)
    {
        $sql = "delete from proxylist where id=".intval($proxyId);
        $this->db->query($sql);
    }

    public function __checkProxy($name, $port=80)
    {
    ...
    public function newProxy($listInfo='')
    {
    ...
    public function createProxy($listInfo)
    {
    ...
    public function insertProxy($listInfo)
    {
    ...
    public function __getProxyInfo($proxyId)
    {
    ...
    public function editProxy($proxyId, $listInfo='')
    {
        if (!empty($proxyId)) {
            if (empty($listInfo)) {
    ...
    public function updateProxy($listInfo)
    {
        $this->set('post', $listInfo);
        $errMsg['proxy'] = formatErrorMsg($this->validate->checkBlank($listInfo['proxy']));
        $errMsg['port'] = formatErrorMsg($this->validate->checkNumber($listInfo['port']));
    ...
    public function __isProxyActive($proxyId)
    {
    ...
    public function checkStatus($proxyId)
    {
     ...
     */
    public function showProxyPerfomance($info = '')
    {
