...
class CronController extends Controller
{
    ...
    public $cronList;                // the array includes all tools avialable for cron
    public $repTools;                // the array includes all tools avialable for report generation
    public $debug = true;            // to show debug message or not
    public $layout = 'ajax';       // ajax layout or not
    public $timeStamp;             // timestamp for storing reports
    public $checkedKeywords = 0;   // the number of keywords checked in cron, this is used for split cron execution feature       	

    # function to load all tools required for report generation 
    public function loadReportGenerationTools($includeList=array())
    {
        $sql = "select * from seotools where status=1 and reportgen=1";
        if (count($includeList) > 0) {
            $sql .= " and id in (".implode(',', $includeList).")";
        }
        $this->repTools = $this->db->select($sql);
    }
    
    # function to load all tools required for cron job
    public function loadCronJobTools($includeList=array())
    {
    ...
    public function showReportGenerationManager()
    {
        $userId = isLoggedIn();
    ...
    public function executeReportGenerationScript($info='')
    {
    ...
    public function executeCron($includeList=array())
    {
    ...
    public function routeCronJob($websiteId, $repTools='', $cron=false)
    {
    ...
    public function saturationCheckerCron($websiteId)
    {
    ...
    public function backlinkCheckerCron($websiteId)
    {
    ...
    public function rankCheckerCron($websiteId)
    {
    ...
    public function keywordPositionCheckerCron($websiteId)
    {
