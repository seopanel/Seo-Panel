...
class SiteAuditorController extends Controller
{
    ...
    public $cron = false;                    // to identify whether it is executed through cron
    public $seArr = array('google', 'bing'); // the array contains search engines 

    public function showAuditorProjects($info="")
    {
        $userId = isLoggedIn();
        if (isAdmin()) {
    ...
    public function newProject($info='')
    {
        $userId = isLoggedIn();
    ...
    public function createProject($listInfo)
    {
        $userId = isLoggedIn();
    ...
    public function checkExcludeLinks($excludeLinks, $websiteUrl, $exclude=true)
    {
    ...
    public function isProjectExists($websiteId, $projectId=false)
    {
    ...
    public function __getProjectInfo($projectId)
    {
    ...
    public function editProject($projectId, $listInfo='')
    {
        $userId = isLoggedIn();
    ...
    public function updateProject($listInfo)
    {
        $userId = isLoggedIn();
        $listInfo['website_id'] = intval($listInfo['website_id']);
        $listInfo['max_links'] = intval($listInfo['max_links']);
    ...
    public function __changeStatus($projectId, $status)
    {
        $projectId = intval($projectId);
    ...
    public function __deleteProject($projectId)
    {
        // delete teh project
        $projectId = intval($projectId);
    ...
    public function getCountcrawledLinks($projectId, $statusCheck=false, $statusVal=1, $conditions='')
    {
    ...
    // function to get number of links of a project
    public function getProjectLastUpdate($projectId)
    {
    ...
    public function showRunProject($projectId)
    {
        $projectId = intval($projectId);
    ...
    public function loadReportsPage($info='')
    {
    ...
    public function checkPageScore($info='')
    {
    ...
    public function __deleteReportPage($reportId)
    {
        if (!empty($reportId)) {
    ...
    public function recheckReportPages($projectId)
    {
        $projectId = intval($projectId);
        if (!empty($projectId)) {
    ...
    public function runProject($projectId)
    {
        ...
        if ($reportUrl = $this->getProjectRandomUrl($projectId)) {
            ...
            if (!$crawlUrl = $this->getProjectRandomUrl($projectId)) {
                ...
                if (!$this->cron) {
                    updateJsLocation('crawling_url', $reportUrl);
                }
            }
        } else {
            $completed = 1;
    ...
    public function getProjectRandomUrl($projectId)
    {
    ...
    public function viewReports($info='')
    {
        $userId = isLoggedIn();
    ...
    public function showProjectReport($data='')
    {
    ...
    public function showLinksReport($data, $exportVersion, $projectInfo)
    {
    ...
    public function viewPageDetails($info='')
    {
        $reportId = intval($info['report_id']);
        if (!empty($reportId)) {
            $auditorComp = $this->createComponent('AuditorComponent');
    ...
    public function showReportSummary($data, $exportVersion, $projectInfo)
    {
        $projectInfo['total_links'] = $this->getCountcrawledLinks($projectInfo['id']);
        $projectInfo['crawled_links'] = $this->getCountcrawledLinks($projectInfo['id'], true);
    ...
    // function to show reports summary of a project
    public function showDuplicateMetaInfo($data, $exportVersion, $projectInfo)
    {
    ...
    public function showCronCommand()
    {
    ...
    public function showImportProjectLinks($info='')
    {
        $userId = isLoggedIn();
        $where = isAdmin() ? "" : " and w.user_id=$userId";
    ...
    public function importLinks($listInfo)
    {
        $userId = isLoggedIn();
