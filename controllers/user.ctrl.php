...
class UserController extends Controller
{
    ...
    public function setLoginSession($userInfo)
    {
    ...
    # func to show users
    public function listUsers($info='')
    {
    ...
    public function __changeStatus($userId, $status)
    {
    ...
    # func to change status
    public function __deleteUser($userId)
    {
        $userId = intval($userId);
    ...
    public function __checkUserName($username)
    {
    ...
    public function __getUserInfo($userId)
    {
        $userId = intval($userId);
    ...
    public function __getAdminInfo()
    {
        $sql = "select * from users where utype_id=1";
        $userInfo = $this->db->select($sql, true);
        return empty($userInfo['id']) ? false :  $userInfo;
    ...
    public function __getAllUsers($active=1, $admin=true)
    {
    ...
    public function __getAllUsersHavingWebsite($active=1, $admin=true)
    {
    ...
    public function createUser($userInfo, $renderResults = true)
    {
    ...
    public function updateUser($userInfo, $renderResults = true)
    {
        $userInfo = sanitizeData($userInfo);
    ...
    public function showMyProfile($info = '')
    {
        $userId = isLoggedIn();
        if (!empty($userId)) {
            $userInfo = $this->__getUserInfo($userId);
    ...
    public function renewMyProfile($info = '')
    {
        $userId = isLoggedIn();
        $seopluginCtrler =  new SeoPluginsController();
    ...
    public function updateSubscription($userInfo = '')
    {
        $userId = isLoggedIn();
        $seopluginCtrler =  new SeoPluginsController();
    ...
    public function editMyProfile($userInfo='')
    {
        $userId = isLoggedIn();
        if (!empty($userId)) {
            if (empty($userInfo)) {
    ...
    public function updateMyProfile($userInfo)
    {
        $userInfo = sanitizeData($userInfo);
        $userId = isLoggedIn();
        $this->set('post', $userInfo);
    ...
    public function requestPassword($userEmail)
    {
    ...
    public function isUserExpired($userId)
    {
    ...
    public function isAdminUserId($userId)
    {
    ...
    public function updateUserInfo($userId, $col, $value)
    {
    ...
    public function calculateUserExpiryDate($quantity)
    {
